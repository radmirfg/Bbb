...
while True:
	try:
		...
		doc = types.InlineKeyboardButton("📕 Руководство", url=f'{bot_documentation}')
		shop = types.InlineKeyboardButton("🛒 Магазин", callback_data='shop')
		snoser = types.InlineKeyboardButton("🤖 BotNet-Snos", callback_data='snoser')
		menu.add(profile)
		menu.add(doc, shop)
		menu.add(snoser)
		...
		sub_3 = types.InlineKeyboardButton(f"🔻 14 дней - {subscribe_14_days}$", callback_data='sub_3')
		sub_4 = types.InlineKeyboardButton(f"🔻 30 дней - {subscribe_30_days}$", callback_data='sub_4')
		sub_5 = types.InlineKeyboardButton(f"🔻 365 дней - {subscribe_365_days}$", callback_data='sub_5')
			...
			for session in sessions:
				...
				try:
					...
					try:
						...
						print(f"Сессия {session} не валид: время подключения истекло.")
						ne_valid += 1
						continue
		...
		def welcome(message):
			...
			bot.send_message(message.chat.id, f'💀 *{bot_name}* — _мощный и безжалостный инструмент сn0са аккаунтов в Telegram._\n\n⚡️ *Админ: {bot_admin}*\n⭐️ *Отзывы:* [Reviews]({bot_reviews})\n🔥 *Работы:* [Works]({bot_works})', parse_mode="Markdown", reply_markup=menu)
			connect.close()
				...
				if not check_user_in_db(user_id):
					...
					if call.message:
						if call.data == 'snoser':
							...
							bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
								text=f'*💀 {bot_name}* — _мощный и безжалостный инструмент сn0са аккаунтов в Telegram._ \n\n*⚡️ Админ: {bot_admin}*\n*⭐️ Отзывы:* [Reviews]({bot_reviews})\n*🔥 Работы:* [Works]({bot_works})',
